...
void PCA9539PW::setInvertReg(unsigned int memAddr, unsigned char polarity = 0x00, uint8_t verbose = 0) {
    ...
    if(verbose > 2) {
        std::cout << "\tPCA9539PW - Setting register " << (int)memAddr + 4 << " to " << std::hex << (int)polarity << std::dec
                  << std::endl;
    }
...
void PCA9539PW::setIOReg(unsigned int memAddr, unsigned char direction = 0xFF, uint8_t verbose = 0) {
    ...
    if(verbose > 2) {
        std::cout << "\tPCA9539PW - Setting register " << (int)memAddr + 6 << " to " << std::hex << (int)direction
                  << std::dec << std::endl;
    }
...
void PCA9539PW::setOutputs(unsigned int memAddr, unsigned char direction = 0xFF, uint8_t verbose = 0) {
    ...
    if(verbose > 2) {
        std::cout << "PCA9539PW - Setting register " << (int)memAddr + 2 << " to " << std::hex << (int)direction << std::dec
                  << std::endl;
    }
    direction = direction & 0xFF;
...
char Si5345::readRegister(unsigned int myaddr, uint8_t verbose) {
    ...
    if(verbose > 2) {
        std::cout << "REG" << std::hex << (int)myaddr << " CURR PG " << (int)currentPg << " REQ PG " << (int)requirePg
                  << std::dec << std::endl;
    }
...
std::vector<std::vector<unsigned int>> Si5345::parseClkFile(const std::string& filename, uint8_t verbose) {
    ...
    if(myfile.good()) {
        ...
        std::cout
            << "\tSi5345 - ERROR: Problem with the configuration file. Make sure the file exists and the path is correct."
            << std::endl;
        return regSetting;
    }
...
void Si5345::writeConfiguration(std::vector<std::vector<unsigned int>> regSetting, uint8_t verbose) {
    ...
    for(int iRow = 0; iRow < nRows; iRow++) {
        ...
        std::cout << std::dec << "\r\t" << std::setw(3) << std::setfill('0') << iRow + 1 << "/" << regSetting.size()
                  << std::flush;

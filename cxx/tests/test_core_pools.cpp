...
TEST_CASE("Message callback", "[core][core::pools]") {
    ...
    const auto mocked_service = MockedChirpService(sender.getName(), CHIRP::MONITORING, sender.getPort());
...
TEST_CASE("Dispose", "[core][core::pools]") {
    ...
    chirp_manager->forgetDiscoveredServices(MD5Hash(sender.getName()));
...
TEST_CASE("Sending and receiving subscriptions", "[core][core::pools]") {
    ...
    const auto mocked_service1 = MockedChirpService(sender1.getName(), CHIRP::MONITORING, sender1.getPort());
    auto sender2 = CMDPSender("CMDPSender.s2");
    const auto mocked_service2 = MockedChirpService(sender2.getName(), CHIRP::MONITORING, sender2.getPort());

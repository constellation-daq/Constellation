...
void Controller::callback_impl(const constellation::chirp::DiscoveredService& service, chirp::ServiceStatus status) {
    ...
    if(status == chirp::ServiceStatus::DEPARTED || status == chirp::ServiceStatus::DEAD) {
        ...
        if(it != connections_.end()) {
            ...
            LOG(logger_, DEBUG) << "Satellite " << it->first << " at " << uri << " departed";
        ...
        try {
            ...
            if(!success) {
                ...
                LOG(logger_, DEBUG) << "Registered remote satellite " << name << " at " << uri;
        ...
        try {
            ...
            try {
                ...
                if(state == CSCP::State::RUN && header.hasTag("last_changed")) {
                    ...
                    LOG(logger_, DEBUG) << "Run started for " << header.getSender() << " at " << to_string(timestamp);
...
std::map<std::string, CSCP1Message> Controller::sendCommands(CSCP1Message& cmd) {
    ...
    for(auto& [name, sat] : connections_) {
        ...
        futures.emplace(
            name, std::async(std::launch::async, [this, sat_copy, &cmd]() { return send_receive(*sat_copy, cmd, true); }));
    }
...
std::map<std::string, CSCP1Message> Controller::sendCommands(const std::string& verb,
    ...
    for(auto& [name, sat] : connections_) {
        ...
        const auto sat_name = name;
        auto* sat_copy = &sat;

        // Start command sending and store future:
        futures.emplace(name, std::async(std::launch::async, [this, sat_copy, sat_name, &verb, &payloads]() {
                            // Prepare message:
                            auto send_msg = (payloads.contains(sat_name)
                                                 ? build_message(verb, payloads.at(sat_name))
                                                 : CSCP1Message({controller_name_}, {CSCP1Message::Type::REQUEST, verb}));
                            return send_receive(*sat_copy, send_msg);
                        }));
    }

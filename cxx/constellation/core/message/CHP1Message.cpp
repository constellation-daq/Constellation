...
CHP1Message CHP1Message::disassemble(zmq::multipart_t& frames) {
    ...
    try {
        ...
        const auto msgpack_flags = msgpack::unpack(to_char_ptr(frame.data()), frame.size(), offset);
        const auto flags = static_cast<CHP::MessageFlags>(msgpack_flags->as<std::uint8_t>());
...
zmq::multipart_t CHP1Message::assemble() {
    ...
    // then flags
    msgpack::pack(sbuf, std::to_underlying(flags_));
    // then interval
    ...
    if(status_.has_value()) {
        ...
        msgpack::pack(sbuf_status, status_.value());
